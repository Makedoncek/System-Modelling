import task_1.random_gen as random_gen
...
class Element:
    ...
    def choose_by_priority(self):
        ...
        for p in range(len(priorities)):
            ...
            # знайти елемент за максимальним пріоритетом
    ...
    def get_delay(self):
        if 'exp' == self.distribution:
            return random_gen.exp(self.delay_mean)
        elif 'norm' == self.distribution:
            return random_gen.norm(self.delay_mean, self.delay_dev)
        elif 'uniform' == self.distribution:
            return random_gen.uniform(self.delay_mean, self.delay_dev)
        else:
